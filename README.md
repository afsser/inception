...
There is a Diagram to show that:
Inside the computer host (a VM in this case) there is the Docker network.
Inside it there are 3 containers, DB, that comunicates through port 3306 with WP+PHP and the DB Volume itselfs that is outside the network, but inside the HOST. the WP+PHP also comunicates with WordPress Volume that is outside the network but inside de HOST and with NGINX through port 9000. And NGINX comunicates with the web through 443.
